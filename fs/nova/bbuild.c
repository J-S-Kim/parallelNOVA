		...
		struct nova_inode_info_header *sih, u16 i_mode)
{
	...
	int cpu_num = smp_processor_id() % sbi->cpus;
	int i;
...
static int nova_init_blockmap_from_inode(struct super_block *sb)
{
	...
	int ret = 0;
	int i;
	int cpu = nova_get_cpuid(sb);
	/* FIXME: Backup inode for BLOCKNODE */
	ret = nova_get_head_tail(sb, pi, &sih, cpu);
	if (ret)
		goto out;

	sih.ino = NOVA_BLOCKNODE_INO;
	for (i = 0 ; i < sbi->cpus ; i++){	
		curr_p = sih.percpu_log_head[i].log_head;
		if (curr_p == 0) {
			nova_dbg("%s: pi head is 0!\n", __func__);
			return -EINVAL;
		}

		while (curr_p != sih.percpu_log_head[i].log_tail) {
...
void nova_save_inode_list_to_log(struct super_block *sb)
{
	...
	pi->percpu_log_head = new_block;
	nova_update_tail(pi, temp_tail, 0);
	nova_flush_buffer(&pi->percpu_log_head, CACHELINE_SIZE, 0);
...
{
	nova_traverse_inode_log(sb, pi, bm, pi->percpu_log_head);
	if (metadata_csum)
